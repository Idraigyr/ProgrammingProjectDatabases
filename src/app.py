...
def setup(app: Flask):
    ...
    with app.app_context():
        ...
        try:
            check_db_schema()
        except AutogenerateDiffsDetected as ex:
            logging.warning(f"AutogenerateDiffsDetected: {ex}")
            logging.warning("Please run 'flask db upgrade' to upgrade the database scheme if not done already.")
            if app.config.get('APP_DEBUG', "false") == "false":
                logging.critical("Refusing to run in production mode. Migrate the database first")
                exit(1)

        if app.config.get('APP_DEBUG', "false") == "true":
            # Create the tables in the db only when in debug mode
            # This would be done by the migration manager in production
            db.create_all()
